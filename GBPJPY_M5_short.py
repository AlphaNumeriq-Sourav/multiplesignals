...
def entry_signal1(data,choice,index):
 ...
 open[2] > close[9]; close[6] > close[7]; (open[0] >= low[1]) AND (open[0] <= min(open[1][1];close[1][1])); low[0] < EMA(close;100)[0]; SuperSmoother[0] <= SuperSmoother[1]; ADX(20)[0] <= ADX(20)[5]                
    ...
    if choice == 1:
        """
            close[0] > high[1]; open[1] > close[4]; close[1] > low[7]; low[3] > high[6]; AvgTrueRange(10)[0] < AvgTrueRange(50)[0]; CCI(20)[0] > CCI(20)[4]            
        
        """
        condition =  (data.iloc[index]['close'] > data.iloc[index-1]['high']) and (data.iloc[index-1]['open'] > data.iloc[index-4]['close']) and ((data.iloc[index-1]['close'] ) > (data.iloc[index-7]['low'] )) \
        and  ((data.iloc[index-3]['low'] ) > (data.iloc[index-6]['high']  )) and (data.iloc[index]['ATR_10'] < data.iloc[index]['ATR_50']) and \
        (data.iloc[index]['CCI_20'] > data.iloc[index-4]['CCI_20'])
    elif choice == 2:
        """
        high[1] > high[3]; open[1] > high[3]; ValueHigh(5)[0] > ValueHigh(5)[3]; ValueHigh(5)[3] > ValueOpen(5)[4]; IBR[0] >= 80; rsi(close;2)[0] crosses above 95
        """

        condition =  (data.iloc[index - 1]['high'] > data.iloc[index-3]['high']) and (data.iloc[index - 1]['open'] > data.iloc[index-3]['high']) and (data.iloc[index]['ValueHigh'] > data.iloc[index-3]['ValueHigh'])\
        and ( data.iloc[index-3]['ValueHigh'] > data.iloc[index-4]['ValueOpen']) and (data.iloc[index]['IBR'] >= 80) and\
              ((data.iloc[index]['rsi_2'] > 95) and ((data.iloc[index-1]['rsi_2'] <= 95)))
    elif choice == 3:
        # open[0] > close[2]; close[2] > open[8]; high[3] > low[7]; pivotpoint[0] > pivotpoint[2]; close[0] >= BollingerBand(c;20;2)[0]; SMA(close;200)[0] < SMA(close;200)[3]

        condition =  (data.iloc[index]['open'] > data.iloc[index-2]['close']) and (data.iloc[index - 2]['close'] > data.iloc[index-8]['open']) and (data.iloc[index - 3]['high'] > data.iloc[index-7]['low'])\
        and (data.iloc[index]['pp'] > data.iloc[index-2]['pp']) and (data.iloc[index]['close'] >= data.iloc[index]['b_upper']) and\
              (data.iloc[index]['sma_200'] < data.iloc[index-3]['sma_200'])
    elif choice == 4:
        # close[1] > close[5]; open[4] > low[7]; close[8] > close[9]; close[0] > EMA(close;20)[0]; open[0] >= highest(open;20)[0]; IBR[0] >= 85        
        condition = (data.iloc[index-1]['close'] > data.iloc[index-5]['close']) and (data.iloc[index - 4]['open'] > data.iloc[index-7]['low']) and (data.iloc[index - 8]['close'] > data.iloc[index-9]['close'])\
         and (data.iloc[index]['close'] > data.iloc[index]['ema_20']) and (data.iloc[index]['open'] > data.iloc[index-1]['highest_o_20'] ) and\
            (data.iloc[index]['IBR'] >= 85)
    elif choice == 5:
        # close[4] > high[8]; low[8] <= high[9]; ValueLow(5)[2] > ValueLow(5)[3]; high[0] >= highest(high;10)[0]; CompositeRSI(2;24)[0] <= 80; SMA(close;200)[0] < SMA(close;200)[3]
        condition = (data.iloc[index-4]['close'] > data.iloc[index-8]['high']) and (data.iloc[index - 8]['low'] <= data.iloc[index-9]['high']) and (data.iloc[index - 2]['ValueLow'] > data.iloc[index-3]['ValueLow'])\
         and (data.iloc[index]['high'] >= data.iloc[index]['highest_h_10']) and (data.iloc[index]['composite_rsi'] <= 80 ) and\
            (data.iloc[index]['sma_200'] < data.iloc[index-3]['sma_200'])
...
def Execution(script_name,symbol , PerCentageRisk , SL_TpRatio ,TP,SL,pipval,logger):
    ...
    for i in range(2):
        try: 
            ...
            df['b_upper'] = sig.BollingerBand(df,20,2)
            df['highest_h_10'] = sig.highest(df,10,'high')
            df['highest_o_20'] = sig.highest(df,20,'open')
            df['IBR'] = sig.IBR(df)
            df['pp'] = sig.pivotpoint(df)
