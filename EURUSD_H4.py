...
def entry_signal1(data,choice,index):
    ...
    if choice == 1:
        """
         ...
         """
        condition =  (data.iloc[index-1]['low'] > data.iloc[index-7]['close']) and (data.iloc[index-4]['close'] > data.iloc[index-9]['high']) and ((data.iloc[index-1]['low'] ) <= (data.iloc[index-3]['low'] )) \
        and  ((data.iloc[index-1]['open'] ) <= (data.iloc[index-4]['open']  )) and (data.iloc[index-1]['ValueOpen'] <= data.iloc[index-4]['ValueOpen']) and \
        (data.iloc[index]['Compositersi'] >= 10)
            ...
            if df_entry.empty:
                for i in range(1,2):
                    ...
                    if entry_signal1(df,i,index):
                        ...
                        order= market_order(symbol , LotSize,'buy',signal,3000+i )
                        tick = mt5.symbol_info_tick(symbol)
                        Price = tick.ask
                        SL_dis = SL
