...
def entry_signal1(data,choice):
    ...
    if choice == 1:
        ...
        # return False
        return  (data.iloc[index]['low'] <= data.iloc[index - 1]['low']) and  (data.iloc[index]['high'] <= data.iloc[index-2]['low']) and  (data.iloc[index]['low'] <= data.iloc[index-4]['high'])\
            and ( data.iloc[index]['open'] <= data.iloc[index-6]['high']) and (data.iloc[index-4]['high'] <= data.iloc[index-9]['close'] )and  data.iloc[index]['tick_volume'] >= data.iloc[index]['sma_50']
...
def Execution(script_name,symbol , PerCentageRisk , SL_TpRatio ,TP,SL,pipval,logger):
    ...
    for i in range(2):
        try: 
            ...
            df['IBR'] = sig.IBR(df)
            df['sma_50'] = sig.SMA(df, 50,'tick_volume')
