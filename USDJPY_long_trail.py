...
low[0] <= close[6]; high[4] <= close[7]; WinsLast(close;20)[0] >= 5; AvgTrueRange(10)[0] <= AvgTrueRange(10)[1]; KaufmanEfficiencyRatio(10)[0] < KaufmanEfficiencyRatio(10)[4]; momentum(close;5)[0] > momentum(close;5)[5]            
    ...
    if choice == 1:
        """
            close[0] > close[4]; close[0] > low[9]; open[2] <= low[7]; close[4] <= close[6]; high[5] <= high[8]; ValueHigh(5)[4] > ValueClose(5)[5]   
        """
        condition =  (data.iloc[index]['close'] > data.iloc[index-4]['close']) and (data.iloc[index]['close'] > data.iloc[index-9]['low']) and \
            ((data.iloc[index-2]['open'] ) <= (data.iloc[index-7]['low'] )) \
        and  ((data.iloc[index-4]['close'] ) <= data.iloc[index-6]['close']) and (data.iloc[index-5]['high'] <= data.iloc[index-8]['high']) and (data.iloc[index-4]['ValueHigh'] > data.iloc[index-5]['ValueClose'] )
        variable = 'ATR'

    elif choice == 2:
        """
        low[0] <= close[6]; high[4] <= close[7]; WinsLast(close;20)[0] >= 5; AvgTrueRange(10)[0] <= AvgTrueRange(10)[1]; KaufmanEfficiencyRatio(10)[0] < KaufmanEfficiencyRatio(10)[4]; 
        momentum(close;5)[0] > momentum(close;5)[5]       
         """
        
        condition =  (data.iloc[index]['low'] <= data.iloc[index-6]['close']) and (data.iloc[index-4]['high'] <= data.iloc[index-7]['close']) and ((data.iloc[index]['Winslast_20'] >= 5))\
        and (data.iloc[index]['ATR_10'] <= data.iloc[index-1]['ATR_10']) and (data.iloc[index]['kauf_10'] < data.iloc[index-4]['kauf_10']) and (data.iloc[index]['mom_5'] > data.iloc[index-5]['mom_5'] )
        ...
        """
        condition =  (data.iloc[index-8]['high'] > data.iloc[index-9]['high']) and (data.iloc[index-3]['low'] <= data.iloc[index-7]['low']) and ((data.iloc[index]['ValueHigh'] > data.iloc[index-2]['ValueLow']))\
        and (data.iloc[index-2]['ValueLow'] <= data.iloc[index-5]['ValueClose']) and (data.iloc[index]['close'] >= data.iloc[index]['compositesma']) \
            and (data.iloc[index]['compositesma'] > data.iloc[index-2]['compositesma'] )
...
def Execution(script_name,symbol , PerCentageRisk , SL_TpRatio ,TP,SL,pipval,logger):
    ...
    for i in range(2):
        try: 
            ...
            df['Winslast_20'] = sig.WinsLast(df,20)
            df['kauf_10'] = sig.KaufmanEfficiencyRatio(df,10,1/SL_TpRatio)
                ...
                for i in range(1,4):
                    ...
                    if condition:
                        ...
                        if variable == 'ATR':
                            ...
                            order= market_order(symbol , LotSize,'buy',signal,4000+i )
                            tick = mt5.symbol_info_tick(symbol)
                            Price = tick.ask
                            SL_dis = SL
                            ...
                            flag = 2
                            time1 = (datetime.fromtimestamp(mt5.symbol_info_tick(symbol).time)- timedelta(hours=3))
                            logger.info(f'Got Indication Candle for fixed model {signal} {symbol} at {time1}')
                            print(f'Got Indication Candle  for signal  {symbol} at {time1}')
                            # index = -2
                            data = df.copy()
                            logger.info(f"{data.iloc[index]['close']} atr: {data.iloc[index]['ATR']} ")
