...
def entry_signal1(data,choice,index):
    ...
    if choice == 1:
        ...
        """
        condition =  (data.iloc[index-7]['high'] > data.iloc[index-9]['low']) and (data.iloc[index-3]['ValueOpen'] > data.iloc[index-4]['ValueClose']) and \
            ((data.iloc[index-1]['ValueLow'] ) <= (data.iloc[index-3]['ValueHigh'] )) \
        and  ((data.iloc[index-2]['ValueLow'] ) <= data.iloc[index-5]['ValueLow']) and (data.iloc[index]['close'] > data.iloc[index]['sma_20']) and (data.iloc[index]['rsi_14'] >= 40)
        variable = 'Trail_ATR'
    ...
    """
        ...
        condition =  (data.iloc[index]['open'] > data.iloc[index-6]['low']) and (data.iloc[index]['OHLC'] > data.iloc[index-1]['OHLC']) and ((data.iloc[index-2]['ValueOpen'] > data.iloc[index-3]['ValueLow']))\
        and (data.iloc[index-1]['ValueLow'] <= data.iloc[index-2]['ValueLow']) and (data.iloc[index-4]['ValueLow'] <= data.iloc[index-5]['ValueClose']) and (data.iloc[index]['open'] < data.iloc[index]['sma_5'] )
                ...
                for i in range(1,3):
                    ...
                    if condition:
                        ...
                        if variable == 'ATR':
                            ...
                            order= market_order(symbol , LotSize,'buy',signal,3001+i )
                            tick = mt5.symbol_info_tick(symbol)
                            Price = tick.ask
                            SL_dis = SL
