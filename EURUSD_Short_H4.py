...
# -------------------------x-----------------------x---------------------------x---------------------------x---------------------------x---------------------------
# Function to Get H4 Forex Signals for EUR 
...
def Execution(script_name,symbol , PerCentageRisk , SL_TpRatio ,TP,SL,pipval,logger):
    ...
    logger.info(f'Running Event Loop for {symbol} at {datetime.now()} for the New Short Signal Modified by Sourav')
                                ...
                                try:
                                    if index not in rows_to_delete:
                                            ...
                                            if Price >= row['SL']:
                                                ...
                                                row['flag'] = 1
                                                logger.info(f"Trail 1  hit Short {time_s} SL at {row['SL']} PT {row['TP']},{row['signals']}  ")
                                            elif (Price <= row['TP']) and (row['flag'] == 1):
                                                time_s = (datetime.fromtimestamp(mt5.symbol_info_tick(symbol).time) - timedelta(hours=3))
                                                row['SL'] = row['SL'] - 1*(SL_TpRatio)
                                                row['TP'] = row['TP'] - 1*(SL_TpRatio)
                                                logger.info(f"Trail next  hit Short {time_s} SL at {row['SL']} PT {row['TP']},{row['signals']} ")
