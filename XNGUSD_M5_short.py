...
def entry_signal1(data,choice,index):
    """
        high[3] <= close[6]; ValueLow(5)[2] > ValueClose(5)[5]; CompositeRSI(2;24)[0] crosses above 50; momentum(close;3)[0] > momentum(close;3)[2]; Stochastics(14)[0] >= 20; SMA(close;8)[0] < SMA(close;8)[5]
        low[0] > high[5]; low[1] > low[9]; high[4] > close[7]; low[2] <= low[3]; low[4] <= high[8]; close[0] < SMA(close;200)[0]
        open[3] > open[8]; close[6] <= close[8]; ValueLow(5)[3] > ValueLow(5)[5]; ValueLow(5)[2] <= ValueLow(5)[5]; Stochastics(14)[0] >= 20; Stochastics(14)[0] >= 60
        open[0] > open[9]; high[1] > open[7]; close[5] > close[8]; open[0] >= highest(open;5)[0]; CCI(20)[0] > CCI(20)[1]; open[0] <= CompositeEMA(8;20;50;200)[0]
        high[1] > high[4]; close[3] > open[7]; close[4] > open[6]; rateOfChange(close;3)[0] > rateOfChange(close;3)[2]; momentum(close;3)[0] > momentum(close;3)[3]; SMA(close;20)[0] < SMA(close;20)[3]
        
        close[0] > close[1]; open[3] > close[6]; open[6] > low[7]; EMA(close;5)[0] > EMA(close;5)[3]; EMA(close;200)[0] < EMA(close;200)[2]; close[0] >= KeltnerChannel(c;20;1.5)[0]
        close[0] > close[9]; close[1] > high[2]; low[1] > open[8]; open[2] > high[8]; high[7] > open[9]; open[0] < EMA(close;200)[0]
        close[1] > high[9]; low[2] > close[7]; open[3] > low[4]; open[4] <= close[6]; ValueClose(5)[1] > ValueClose(5)[3]; ValueHigh(5)[0] <= ValueHigh(5)[5]
        open[4] > open[5]; high[1] <= low[3]; low[1] <= low[3]; open[1] <= open[6]; close[0] > s2[0]; (rsi(close;14)[0] > 50) AND (rsi(close;14)[0] <= 60)
        open[1] > low[4]; close[2] > high[4]; ValueClose(5)[1] > ValueClose(5)[3]; CompositeRSI(2;24)[0] >= 60; momentum(close;3)[0] > momentum(close;3)[4]; EMA(close;200)[0] < EMA(close;200)[2]
        open[1] > low[4]; close[3] > high[9]; high[7] > low[8]; WinsLast(close;5)[0] >= 4; momentum(close;5)[0] > 0; EMA(close;200)[0] < EMA(close;200)[5]
        close[0] > close[2]; high[5] > open[7]; high[0] < EMA(close;200)[0]; close[0] > EMA(close;8)[0]; barPath[0] = 8; SMA(close;20)[0] <= SMA(close;200)[0]            
    """
    index = index
    if choice == 1:
        """
        ...
        """
        condition = (data.iloc[index-3]['high'] <= data.iloc[index-6]['close']) and (data.iloc[index-2]['ValueLow'] > data.iloc[index-5]['ValueClose']) and \
            ((data.iloc[index]['composite_rsi']  > 50) and (data.iloc[index-1]['composite_rsi']  <= 50)) \
        and  ((data.iloc[index]['mom_3'] ) > (data.iloc[index-2]['mom_3']  )) and (data.iloc[index]['stoch_14'] >= 20) and \
        (data.iloc[index]['sma_8'] < data.iloc[index-5]['sma_8'])
    elif choice == 2:
        """
        low[0] > high[5]; low[1] > low[9]; high[4] > close[7]; low[2] <= low[3]; low[4] <= high[8]; close[0] < SMA(close;200)[0]
        """

        condition = (data.iloc[index]['low'] > data.iloc[index-5]['high']) and (data.iloc[index - 1]['low'] > data.iloc[index-9]['low']) and (data.iloc[index - 4]['high'] > data.iloc[index-7]['close'])\
        and (data.iloc[index - 2]['low'] <= data.iloc[index-3]['low']) and (data.iloc[index-4]['low'] <= data.iloc[index -8]['high']) and\
              (data.iloc[index]['close'] < data.iloc[index]['sma_200'])
    elif choice == 3:
        #         open[3] > open[8]; close[6] <= close[8]; ValueLow(5)[3] > ValueLow(5)[5]; ValueLow(5)[2] <= ValueLow(5)[5]; Stochastics(14)[0] >= 20; Stochastics(14)[0] >= 60

        condition = (data.iloc[index - 3]['open'] > data.iloc[index-8]['open']) and (data.iloc[index - 6]['close'] <= data.iloc[index-8]['close']) and (data.iloc[index - 3]['ValueLow'] > data.iloc[index-5]['ValueLow'])\
            and (data.iloc[index-2]['ValueLow'] <= data.iloc[index-5]['ValueLow']) and  (data.iloc[index]['stoch_14'] >= 20) and (data.iloc[index]['stoch_14'] >= 60)
    elif choice == 4:
        #  open[0] > open[9]; high[1] > open[7]; close[5] > close[8]; open[0] >= highest(open;5)[0]; CCI(20)[0] > CCI(20)[1]; open[0] <= CompositeEMA(8;20;50;200)[0]

        condition = (data.iloc[index]['open'] > data.iloc[index-9]['open']) and (data.iloc[index - 1]['high'] > data.iloc[index-7]['open']) and (data.iloc[index - 5]['close'] > data.iloc[index-8]['close'])\
         and (data.iloc[index]['open'] >= data.iloc[index]['highest_o_5']) and (data.iloc[index]['CCI_20'] > data.iloc[index-1]['CCI_20'] ) and\
            (data.iloc[index]['open'] <= data.iloc[index]['composite_ema'])
    elif choice == 5:
        #         high[1] > high[4]; close[3] > open[7]; close[4] > open[6]; rateOfChange(close;3)[0] > rateOfChange(close;3)[2]; momentum(close;3)[0] > momentum(close;3)[3]; SMA(close;20)[0] < SMA(close;20)[3]

        condition = (data.iloc[index-1]['high'] > data.iloc[index-4]['high']) and (data.iloc[index - 3]['close'] > data.iloc[index-7]['open']) and (data.iloc[index - 4]['close'] > data.iloc[index-6]['open'])\
         and (data.iloc[index]['roc_3'] > data.iloc[index-2]['roc_3']) and (data.iloc[index]['mom_3'] > data.iloc[index-3]['mom_3'] ) and\
            (data.iloc[index]['sma_20'] < data.iloc[index-3]['sma_20'])
    elif choice == 6:
        """
        ...
        """
        condition = (data.iloc[index]['close'] > data.iloc[index-1]['close']) and (data.iloc[index - 3]['open'] > data.iloc[index-6]['close']) and (data.iloc[index - 6]['open'] > data.iloc[index-7]['low'])\
         and (data.iloc[index]['ema_5'] > data.iloc[index-3]['ema_5']) and (data.iloc[index]['ema_200'] < data.iloc[index-2]['ema_200'] ) and\
            (data.iloc[index]['close'] >= data.iloc[index]['kc_upper'])
    elif choice == 7:
        """
        ...
        """
        condition = (data.iloc[index]['close'] > data.iloc[index-9]['close']) and (data.iloc[index - 1]['close'] > data.iloc[index-2]['high']) and (data.iloc[index - 1]['low'] > data.iloc[index-8]['open'])\
         and (data.iloc[index-2]['open'] > data.iloc[index-8]['high']) and (data.iloc[index-7]['high'] > data.iloc[index-9]['open'] ) and\
            (data.iloc[index]['open'] < data.iloc[index]['ema_200'])
    elif choice == 8:
        """
        ...
        """
        condition = (data.iloc[index-1]['close'] > data.iloc[index-9]['high']) and (data.iloc[index - 2]['low'] > data.iloc[index-7]['close']) and (data.iloc[index - 3]['open'] > data.iloc[index-4]['low'])\
         and (data.iloc[index-4]['open'] <= data.iloc[index-6]['close']) and (data.iloc[index-1]['ValueClose'] > data.iloc[index-3]['ValueClose'] ) and\
            (data.iloc[index]['ValueHigh'] <= data.iloc[index-5]['ValueHigh'])
    elif choice == 9:
        """
        ...
        """
        condition = (data.iloc[index-4]['open'] > data.iloc[index-5]['open']) and (data.iloc[index - 1]['high'] <= data.iloc[index-3]['low']) and (data.iloc[index - 1]['low'] <= data.iloc[index-3]['low'])\
         and (data.iloc[index-1]['open'] <= data.iloc[index-6]['open']) and (data.iloc[index]['close'] > data.iloc[index]['s2'] ) and\
            ((data.iloc[index]['rsi_14'] > 50) and (data.iloc[index]['rsi_14'] <= 60))
    elif choice == 10:
        """
        ...
        """
        condition = (data.iloc[index-1]['open'] > data.iloc[index-4]['low']) and (data.iloc[index - 2]['close'] > data.iloc[index-4]['high']) and (data.iloc[index - 1]['ValueClose'] > data.iloc[index-3]['ValueClose'])\
         and (data.iloc[index]['composite_rsi'] >= 60) and (data.iloc[index]['mom_3'] > data.iloc[index-4]['mom_3'] ) and\
            (data.iloc[index]['ema_200'] < data.iloc[index-2]['ema_200'])
    elif choice == 11:
        """
        ...
        """
        condition = (data.iloc[index-1]['open'] > data.iloc[index-4]['low']) and (data.iloc[index - 3]['close'] > data.iloc[index-9]['high']) and (data.iloc[index - 7]['high'] > data.iloc[index-8]['low'])\
         and (data.iloc[index]['winslast_5'] >= 4) and (data.iloc[index]['mom_5'] > 0 ) and\
            (data.iloc[index]['ema_200'] < data.iloc[index-5]['ema_200'])
    elif choice == 12:
        """
        
        close[0] > close[2]; high[5] > open[7]; high[0] < EMA(close;200)[0]; close[0] > EMA(close;8)[0]; barPath[0] = 8; SMA(close;20)[0] <= SMA(close;200)[0]   
        """
        condition = (data.iloc[index]['close'] > data.iloc[index-2]['close']) and (data.iloc[index - 5]['high'] < data.iloc[index]['ema_200']) and (data.iloc[index]['close'] > data.iloc[index]['ema_8'])\
         and (data.iloc[index]['sma_20'] < data.iloc[index-2]['sma_20'] ) and\
            (data.iloc[index]['barpath']  == 8)
...
def Execution(script_name,symbol , PerCentageRisk , SL_TpRatio ,TP,SL,pipval,logger):
    ...
    for i in range(2):
        try: 
            ...
            df['sma_200'] = sig.SMA(df, 200)
            df['sma_20'] = sig.SMA(df, 20)
            df['sma_30'] = sig.SMA(df, 30)
