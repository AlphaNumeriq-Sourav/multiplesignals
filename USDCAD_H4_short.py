...
def entry_signal1(data,choice,index):
    ...
    if choice == 1:
        ...
        """
        condition =  (data.iloc[index-4]['close'] > data.iloc[index-6]['low']) and (data.iloc[index]['open'] <= data.iloc[index-3]['open']) and ((data.iloc[index-4]['ValueLow'] ) > (data.iloc[index-5]['ValueLow'] )) \
        and  ((data.iloc[index]['ValueHigh'] ) <= (data.iloc[index-4]['ValueClose']  )) and (data.iloc[index-3]['ValueHigh'] <= data.iloc[index-5]['ValueClose']) and \
        (data.iloc[index]['ValueLow'] <= data.iloc[index]['ValueClose'])
    elif choice == 2:
        """
        close[0] > high[8]; close[5] > open[6]; high[7] > close[9]; ValueOpen(5)[0] <= 6; ValueLow(5)[0] > -6; KeltnerChannel(c;20;1.5)[0] < KeltnerChannel(c;20;1.5)[1]            
        """

        condition =  (data.iloc[index]['close'] > data.iloc[index-8]['high']) and (data.iloc[index - 5]['close'] > data.iloc[index-6]['open']) and (data.iloc[index-7]['high'] > data.iloc[index-9]['close'])\
        and ( data.iloc[index]['ValueOpen'] <= 6) and (data.iloc[index]['ValueLow'] > -6) and\
              ((data.iloc[index]['kc_upper'] < data.iloc[index-1]['kc_upper'] ))
