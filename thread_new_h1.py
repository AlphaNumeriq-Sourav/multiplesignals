...
def files(script_name,symbol , RISK , ds ,TP,SL,pip):
    ...
    # create file handler which logs even debug messagesk,mm
    fh = logging.FileHandler(f'{script_name}_BA_atr.log')
...
def premain():
    ...
    script_args = {
        ...
        "EURUSD_H1_short": ('EURUSD',0.002 , 0.0001,2,3,10),
        "GBPUSD_H1_short": ('GBPUSD',0.002 , 0.0001,2,3,10)
    }
    # "US500_H1_short": ('US500',0.002 , 0.01,2,3,0.01),
        # "USDCAD_H1_short": ('USDCAD',0.002 , 0.0001,2,3,10),
        # "EURUSD_H1_long": ('EURUSD',0.002 , 0.0001,2,3,10),
        # "GBPUSD_H1_long": ('GBPUSD',0.002 , 0.0001,2,3,10),
    # script_args = {
        
    #     "EURUSD_H1_long_short": ('EURUSD',0.002 , 0.0001,2,3,10)
        ...
        while True:
            if (datetime.now().weekday()) != 5 and    \
                ...
                if mt5.initialize():
                    ...
                    if ((datetime.fromtimestamp(mt5.symbol_info_tick(symbol).time) - timedelta(hours=3)).weekday()) != 5 and        \
                                            ((datetime.fromtimestamp(mt5.symbol_info_tick(symbol).time) - timedelta(hours=3)).weekday()) != 6:
