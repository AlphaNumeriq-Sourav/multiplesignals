...
def entry_signal1(data,choice,index):
    ...
    if choice == 1:
        ...
        """
        condition =  (data.iloc[index-4]['close'] > data.iloc[index-6]['close']) and (data.iloc[index-1]['close'] <= data.iloc[index-2]['high']) and \
            ((data.iloc[index]['Cubehlc'] ) <= (data.iloc[index-4]['Cubehlc'] )) \
        and  ((data.iloc[index-2]['ValueHigh'] ) >= data.iloc[index-5]['ValueHigh']) and (data.iloc[index-3]['ValueLow'] <= data.iloc[index-5]['ValueHigh']) and \
            (data.iloc[index]['ValueClose'] <= data.iloc[index-5]['ValueLow'] )
        variable = 'Fixed'
        ...
        try: 
            ...
            for i in range(5):
                if  (df.iloc[-1]['time'].hour == time_hr.hour) or (df.iloc[-1]['time'].hour > time_hr.hour) or ((df.iloc[-1]['time'].hour == 0) and (time_hr.hour >=23)):
                    ...
                    if  (df.iloc[-1]['time'].hour == time_hr.hour) or (df.iloc[-1]['time'].hour > time_hr.hour) or ((df.iloc[-1]['time'].hour == 0) and (time_hr.hour >=23)):
                     index = -2
                if  (df.iloc[-1]['time'].hour < time_hr.hour) and ( time_hr.hour == 1):
                     index = -2
                     break
                else:
                    df = pd.DataFrame(mt5.copy_rates_from_pos(symbol ,mt5.TIMEFRAME_H4, 0 , 2700))
                ...
                for i in range(1,2):
                    ...
                    if condition:
                        ...
                        if variable == 'ATR':
                            ...
                            LotSize = round((PerCentageRisk * mt5.account_info().equity)/(pipval*2000),2)
                            
                            order= market_order(symbol , LotSize,'sell',signal,6100+i )
                            tick = mt5.symbol_info_tick(symbol)
                            Price = tick.bid
                            SL_dis = SL
                            ATR = df.iloc[index]['ATR']
                            StopLoss = Price + 2000*SL_TpRatio
                            TP_val = Price - 1200*SL_TpRatio
                            order_id = order.order
                            order_price = order.price
                            print(f'Entry at {time1} for    {symbol}  , SL : {StopLoss} , TP : {TP_val} ,Lotsize : {LotSize} ')
                            logger.info(f'Entry at {time1} for {symbol}  , SL : {StopLoss} , TP : {TP_val} ,Lotsize : {LotSize} , OrderPrice : {Price}, comment : {order.comment}  {flag}')
                            ...
                            for index, row in df_entry.iterrows():  
                                try:
                                    if index not in rows_to_delete:
                                            ...
                                            if Price1 >= row['SL']:
                                            ...
                                            elif (Price1 <= row['TP']) and (row['flag'] == 0):
                                                time_s = (datetime.fromtimestamp(mt5.symbol_info_tick(symbol).time) - timedelta(hours=3))
                                            ...
                                            elif (Price1 <= row['TP']) and (row['flag'] == 1):
                                                time_s = (datetime.fromtimestamp(mt5.symbol_info_tick(symbol).time) - timedelta(hours=3))
                                                ...
                                                # TP Hit
                                                # Trailing Triggere
                                            elif (Price1 <= row['TP']) and ((row['flag'] == 2) or (row['flag'] >= 5)):
                                                    time_s = (datetime.fromtimestamp(mt5.symbol_info_tick(symbol).time) - timedelta(hours=3))
