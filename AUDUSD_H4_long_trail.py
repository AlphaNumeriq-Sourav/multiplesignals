...
def entry_signal1(data,choice,index):
    ...
    if choice == 1:
        ...
        """
        condition =  (data.iloc[index-6]['open'] > data.iloc[index-8]['close']) and (data.iloc[index-8]['high'] > data.iloc[index-9]['open']) and ((data.iloc[index-2]['low'] ) <= (data.iloc[index-4]['low'] )) \
        and  ((data.iloc[index-2]['close'] ) <= (data.iloc[index-8]['open']  )) and (data.iloc[index-3]['open'] <= data.iloc[index-5]['open']) and \
        (data.iloc[index]['ValueOpen'] > -2)
        variable = 'ATR'
        ...
        condition =  (data.iloc[index-3]['high'] > data.iloc[index-4]['low']) and (data.iloc[index-6]['high'] > data.iloc[index-9]['low']) and ((data.iloc[index]['open'] <= data.iloc[index-2]['close']))\
        and (data.iloc[index]['low'] <= data.iloc[index-4]['close']) and (data.iloc[index-2]['open'] <= data.iloc[index-6]['open']) and (data.iloc[index]['close'] > data.iloc[index]['ema_3'])\
        and  (data.iloc[index-1]['close'] <= data.iloc[index-1]['ema_3'])
        variable = 'Trail'
                ...
                for i in range(1,3):
                    ...
                    if condition:
                        ...
                        if variable == 'ATR':
                            ...
                            order= market_order(symbol , LotSize,'buy',signal,5000+i )
                            tick = mt5.symbol_info_tick(symbol)
                            Price = tick.ask
                            SL_dis = SL
                            ...
                            df_entry.loc[len(df_entry)] = [i,order_id,order.volume,Price,TP_val,StopLoss,flag]
