...
def entry_signal1(data,choice):
    ...
    if choice == 1:
        """
 open[1] > high[7]; low[2] > low[6]; open[3] > close[6]; high[4] > close[5]; CompositeRSI(2;24)[0] >= 30; close[0] crosses above EMA(close;200)[0]
  
    """         
        return (data.iloc[index-1]['open'] > data.iloc[index-7]['high']) and (data.iloc[index - 2]['low'] > data.iloc[index - 6]['low']) \
                and (data.iloc[index-3]['open'] > data.iloc[index - 6]['close'] )and (data.iloc[index-4]['high'] > data.iloc[index-5]['close'] )  \
            and (data.iloc[index]['composite_rsi'] >= 30)  and ((data.iloc[index]['close'] >= data.iloc[index]['ema_200']) and (data.iloc[index-1]['close'] < data.iloc[index-1]['ema_200'])) 
    elif choice == 2:
        """
                
 high[1] > close[9]; ValueLow(5)[0] > -12; ValueClose(5)[1] > ValueClose(5)[5]; CompositeRSI(2;24)[0] >= 70; SMA(close;100)[0] < SMA(close;100)[4]; EMA(close;3)[0] > EMA(close;3)[5]      
        
        """
        index = -2
        return (data.iloc[index-1]['high'] > data.iloc[index - 9]['close']) and (data.iloc[index ]['ValueLow'] > -12) and (data.iloc[index-1]['ValueClose'] > data.iloc[index-5]['ValueClose']) \
        and (data.iloc[index]['composite_rsi'] >= 70)  and (data.iloc[index]['sma_100'] < data.iloc[index-4]['sma_100']) and (data.iloc[index]['ema_3'] > data.iloc[index-5 ]['ema_3'])

    elif choice == 3:
        """
                    
 high[0] > open[5]; open[2] > open[8]; high[4] > low[5]; close[0] > BodyMedian[0]; close[0] < SMA(close;100)[0]; (CompositeRSI(2;24)[0] >= 65) AND (CompositeRSI(2;24)[0] <= 75)
        """
        index = -2
        return (data.iloc[index]['high'] > data.iloc[index - 5]['open']) and (data.iloc[index-2 ]['open'] > data.iloc[index - 8]['open']) and (data.iloc[index-4]['high'] > data.iloc[index - 5]['low']) \
        and (data.iloc[index]['close'] > data.iloc[index]['BodyMedian'] ) and (data.iloc[index]['close'] < data.iloc[index]['sma_100']) and \
            ((data.iloc[index]['composite_rsi'] >= 65) and (data.iloc[index]['composite_rsi'] <= 75))
    elif choice == 4:
        
        """
 close[2] > open[9]; open[8] > close[9]; close[0] <= close[1]; close[0] <= open[4]; ValueOpen(5)[1] > ValueHigh(5)[5]; ValueHigh(5)[0] > ValueClose(5)[1]
    """     
        index = -2
        return (data.iloc[index-2]['close'] > data.iloc[index-9]['open']) and (data.iloc[index-8]['open'] > data.iloc[index-9]['close']) and (data.iloc[index]['close'] <= data.iloc[index-1]['close'])\
        and (data.iloc[index]['close'] <= data.iloc[index-4]['open']) and (data.iloc[index-1]['ValueOpen'] > data.iloc[index-5]['ValueHigh'])\
        and (data.iloc[index]['ValueHigh'] > data.iloc[index-1]['ValueClose'])
...
def Execution(symbol ,entry ,PerCentageRisk , SL_TpRatio ,TP,SL,pipval,logger):
    ...
    for i in range(2):
        try: 
            ...
            df['BodyMedian'] = sig.BodyMedian(df)
            ...
            if df_entry.empty:
                for i in range(1,5):
                    ...
                    if entry_signal1(df,i):
                        ...
                        logger.info(f'Got Indication Candle for short {signal} {symbol} at {time1}')
                        print(f'Got Indication Candle  for short signal {symbol} at {time1}')
                        index = -2
                        data = df.copy()
                        ...
                        order= market_order(symbol , LotSize,'sell',signal,210+i )
                        tick = mt5.symbol_info_tick(symbol)
                        Price = tick.bid
                        SL_dis = SL
                        ...
                        print(f'Entry at {time1} for  short  {symbol}  , SL : {StopLoss} , TP : {TP_val} ,Lotsize : {LotSize} ')
                        logger.info(f'Entry at {time1} for short {symbol}  , SL : {StopLoss} , TP : {TP_val} ,Lotsize : {LotSize} , OrderPrice : {Price}, comment : {order.comment}')
                        df_entry.loc[len(df_entry)] = [i,order_id,order.volume,Price,TP_val,StopLoss]
                    else:
                        
                        logger.info(f'No entry for short {signal} {(datetime.fromtimestamp(mt5.symbol_info_tick(symbol).time) - timedelta(hours=3))}')    
