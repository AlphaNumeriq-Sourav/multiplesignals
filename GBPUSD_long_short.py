...
def entry_signal1(data,choice):
 ...
 open[2] > close[9]; close[6] > close[7]; (open[0] >= low[1]) AND (open[0] <= min(open[1][1];close[1][1])); low[0] < EMA(close;100)[0]; SuperSmoother[0] <= SuperSmoother[1]; ADX(20)[0] <= ADX(20)[5]                
    ...
    # return True
        ...
        return (data.iloc[index-6]['high'] > data.iloc[index - 9]['high']) and (data.iloc[index - 2]['close'] <= data.iloc[index - 6]['low']) \
            and (data.iloc[index - 3]['low'] <= data.iloc[index-7]['low']) and (data.iloc[index - 3]['low'] <= data.iloc[index-8]['close'])  and (data.iloc[index]['ATR_10'] <= data.iloc[index-5]['ATR']) \
                and (data.iloc[index]['RSI_2']) <= 30
    elif choice == 5:
